...
import { saveImprovementAction, SaveImprovementActionData } from './actions'
...
export function EditForm({ initial }: EditFormProps) {
    ...
    const onSubmit = async (data: FormData) => {
        ...
        const finalData: SaveImprovementActionData = {
            id: initial?.id,
            title: data.title,
            text: data.text,
        ...
        try {
            const result = await saveImprovementAction(finalData)

            if (result.isCreating) {
                toast.success('Запись создана')
                router.push(`/admin/improvement/${result.id}`)
            } else {
    ...
    return (
        <form className="form" onSubmit={handleSubmit(onSubmit)} noValidate>
            {isEditing && (
                <div className="field">
                    <div className="buttons-wrapper">
                        <Link href={`/admin/improvement/${initial.id}/gallery`} className="primary-button">
            ...
            <div className="buttons-wrapper">
                ...
                <Link href="/admin/improvement" className="dangerous-button">
                    Отменить
                </Link>
            </div>
        </form>
